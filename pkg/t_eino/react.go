...
type state struct {
	...
	toolState                map[string]interface{} //工具状态存储
	lock                     *sync.Mutex
}
...
//	if err != nil {...}
//	msg, err := t_eino.Generate(ctx, []*schema.Message{{Role: schema.User, Content: "how to build t_eino with eino"}})
//	if err != nil {...}
//	println(msg.Content)
type Agent struct {
...
func (r *Agent) Stream(ctx context.Context, input []*schema.Message, opts ...agent.AgentOption) (output *react.Iterator[*schema.StreamReader[*schema.Message]], err error) {
	opt, mf := react.WithMessageFuture()
	opts = append(opts, opt)
	_, err = r.runnable.Stream(ctx, input, agent.GetComposeOptions(opts...)...)
	if err != nil {
		return
	}
